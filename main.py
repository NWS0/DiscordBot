...
from aiohttp import web  # 【新增】導入 aiohttp 網頁套件

# --- 【新增】喚醒用虛擬網頁設定 ---
async def web_server_handler(request):
    return web.Response(text="🎵 音樂精靈正在雲端蹦蹦跳跳，沒有偷睡覺喔！")
...
async def start_dummy_server():
    ...
    await runner.setup()
    # Render 會自動給我們一個 PORT 環境變數，沒給的話預設用 10000
    port = int(os.environ.get("PORT", 10000))
    site = web.TCPSite(runner, '0.0.0.0', port)
    await site.start()
    print(f"🌐 喚醒用虛擬網頁已啟動 (Port: {port})")
# ----------------------------------
